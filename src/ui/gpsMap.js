...
export class GpsMap {
  ...
  _rebuildMarkers() {
    const s = this.schema
    const list = []
    for (const e of s.extractions) list.push({ ...e, kind: 'extract', status: 'active' })
    const q = this.ctx && typeof this.ctx.get === 'function' ? this.ctx.get('quests') : null
    const quests = q && typeof q.markers === 'function' ? q.markers(this.mapId) : s.quests
    for (const m of quests) list.push({ radius: 3, ...m, kind: 'quest' })
    this.markers = list
  }
